        ...
        {addonFields.map((field, index) => (
          ...
          >
            ...
            <div className="mt-1 ml-14">
              <Controller
                control={form.control}
                name={`addons.${index}.description_json`}
                render={({ field: descField }) => (
                  <InlineDescriptionEditor
                    value={descField.value ?? form.getValues(`addons.${index}.description`) ?? null}
                    onChange={(json, text) => {
                      descField.onChange(json);
                      form.setValue(`addons.${index}.description`, text);
                    }}
                    onBlur={descField.onBlur}
                    placeholder="Add description for this add-on..."
                  />
                )}
              />
            </div>
